...
class BasePLModule(pl.LightningModule):
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> None:
        ...
        try:
            ...
            self.log("validation_error", str(e))
    ...
    def on_validation_epoch_end(self, output=None) -> Any:
        if not output:
            self.log("val_prec_micro", 0.0)
            self.log("val_recall_micro", 0.0)
            self.log("val_F1_micro", 0.0)
            return
        ...
        try:
            scores, precision, recall, f1 = re_score(
                ...
                ['killed by', 'residence', 'location', 'headquarters location', 'employer']
            )
            self.log('val_prec_micro', precision)
            self.log('val_recall_micro', recall)
            self.log('val_F1_micro', f1)
        except Exception as e:
            self.log("validation_epoch_end_error", str(e))
            self.log("val_prec_micro", 0.0)
            self.log("val_recall_micro", 0.0)
            self.log("val_F1_micro", 0.0)
