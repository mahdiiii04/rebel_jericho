...
#         "additional_special_tokens": ['<obj>', '<subj>', '<triplet>'],
    ...
    # trainer
    trainer = pl.Trainer(
        accelerator="gpu" if conf.gpus else "cpu",  # Use GPU if available, else CPU
        devices=conf.gpus if conf.gpus else 1,
    ...
    # module fit
    trainer.fit(pl_module, datamodule=pl_data_module,ckpt_path=conf.resume_from_checkpoint)
